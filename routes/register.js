...
router.post("/", checkAuthentication.checkNotAuthenticated, (req, res) => {
  ...
  if (!checkBirthday(req.body.birthday)) {
    res.render("register", {
      warning:
        "Η ηλικία που δηλώσατε είναι κάτω τον 18. Δυστυχώς απαγορέυεται η εγγραφή σας.",
      fields: req.body,
    });
    return;
  }
  bcrypt.hash(req.body.telephone, 10, (err, hash) => {
    ...
    let query0 = conn.query(sql0, (err, results0) => {
      ...
      if (!(results0[0] == null)) {
        res.render("register", {
          warning:
            "O χρήστης υπάρχει ήδη. Μπορείτε να συνδεθείτε με το email και το τηλέφωνο που είχατε δηλώσει.",
          fields: req.body,
        });
        return;
      }
